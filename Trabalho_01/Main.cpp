...
int main(int argc, char** argv)
{
  ...
  points[0]  = Point{0.5f, 0.5f, 0.0f};
  points[1]  = Point{0.8f, 2.0f, 0.0f}; // Fora em X
  points[2]  = Point{4.5f, 4.5f, 0.0f}; // Fora em tudo (muito alto)

  // Pontos dentro da caixa de busca [1.0, 1.0, 1.0] ate [3.0, 3.0, 3.0]
  points[3]  = Point{1.5f, 1.5f, 0.0f};
  points[4]  = Point{2.0f, 2.0f, 0.0f};
  points[5]  = Point{2.5f, 2.5f, 0.0f};
  
  // Casos Críticos: Pontos exatamente nas bordas do Bounds para testar intervalos abertos/fechados
  points[6]  = Point{1.0f, 1.0f, 0.0f}; // Borda mínima
  points[7]  = Point{3.0f, 3.0f, 0.0f}; // Borda máxima

  // Casos Ultra Críticos: Coordenadas duplicadas em alguma dimensão (Para testar node.antes / node.depois)
  points[8]  = Point{2.0f, 1.2f, 0.0f}; // Compartilha X=2.0 com o points[4]
  points[9]  = Point{2.0f, 2.9f, 0.0f}; // Compartilha X=2.0 com o points[4] e [8]
  points[10] = Point{1.8f, 2.0f, 0.0f}; // Compartilha Y=2.0 e Z=2.0 com points[4]
  points[11] = Point{2.2f, 2.0f, 0.0f}; // Compartilha Y=2.0 com points[4] e [10]
